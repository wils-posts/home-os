...
export default function LoginView({ auth }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-6 bg-slate-900">
      ...
      <p className="text-slate-400 text-sm mb-8">Sign in to your household</p>
      ...
      {step === 'email' ? (
        <form onSubmit={handleSendOtp} className="w-full max-w-sm flex flex-col gap-3">
          <input
            ...
            inputMode="email"
            autoComplete="email"
            className="h-12 px-4 rounded-xl border border-slate-600 bg-slate-800 text-white placeholder:text-slate-500 text-base focus:outline-none focus:ring-1 focus:ring-slate-500"
          />
          {error && <p className="text-need text-sm">{error}</p>}
          <button
            type="submit"
            disabled={loading || !email.trim()}
            className="h-12 bg-white text-slate-900 rounded-xl font-semibold text-base disabled:opacity-50 active:scale-95 transition-transform"
          >
        ...
        <form onSubmit={handleVerifyOtp} className="w-full max-w-sm flex flex-col gap-3">
          ...
          <input
            ...
            maxLength={6}
            className="h-12 px-4 rounded-xl border border-slate-600 bg-slate-800 text-white text-xl text-center tracking-widest font-mono focus:outline-none focus:ring-1 focus:ring-slate-500"
          />
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-slate-400 text-sm disabled:opacity-50"
            >
